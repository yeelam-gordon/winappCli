...
- Human output and JSON `summary.contrast` report `attempted`, `measured`, and `unmeasured` candidate counts. Per-element native HWND boundaries are preserved so pixels from another surface are never substituted. Every unmeasured eligible candidate is a failure and exits non-zero. A view with no eligible visible text candidates is reported explicitly and is not itself a failure.
