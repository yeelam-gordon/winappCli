...
- Human output and JSON `summary.contrast` report `attempted`, `measured`, and `unmeasured` candidate counts. Source-window boundaries prevent pixels from another top-level window from being substituted, while child native HWND controls remain eligible for the root capture. Every unmeasured eligible candidate is a failure and exits non-zero. A view with no eligible visible text candidates is reported explicitly and is not itself a failure.
