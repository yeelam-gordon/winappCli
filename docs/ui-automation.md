...
- **Named keys** — `enter`/`return`, `numpadenter`/`kpenter`, `divide`/`numpaddivide`/`kpdivide`, `tab`, `esc`/`escape`, `space`, `backspace`, `delete`/`del`, `insert`, `home`, `end`, `pageup`/`pgup`, `pagedown`/`pgdn`, `up`/`down`/`left`/`right`, `f1`–`f16`, `apps`, `printscreen`, `capslock`.
- **Sequences** — multiple tokens are pressed in order: `down down enter`.
- **Modifier combos** — `ctrl`, `shift`, `alt`, `win` joined with `+`: `ctrl+shift+t`, `alt+f4`. Explicit sided forms are also supported: `lctrl`/`rctrl`, `lshift`/`rshift`, `lalt`/`ralt`, and `lwin`/`rwin` (plus `left...`/`right...` spellings).
...
- `--via <transport>` — `post-message` (default) posts `WM_KEYDOWN`/`WM_KEYUP`/`WM_CHAR` to a resolved target HWND; success confirms queueing, not target consumption. `send-input` injects OS-wide via `SendInput` and goes to the foreground window. **Both transports are subject to UIPI** and can target only equal- or lower-integrity processes.
- `--allow-system-keys` — With `--via send-input`, opt in to system-/shell-reserved global shortcuts that are rejected by default. Any Win-modified `L` chord remains blocked, including variants with Shift/Ctrl/Alt; with `post-message` the flag has no effect and emits a warning.

**Choosing a transport / known limits:**
- `post-message` is the default because delivery is scoped to a resolved HWND rather than the global input stream; it does **not** bypass UIPI. A missing or already-stale HWND fails preflight with `no_target_window`. After a target passes that validation, a later `PostMessage` failure returns `input_injection_failed` — including UIPI denial (`ERROR_ACCESS_DENIED` / 5), the target disappearing in the validation-to-post race, or a queue failure. The CLI checks every post and makes a best-effort key-up cleanup if a later message fails after a key-down. Posting is not transactional, so the target may still observe a delivered prefix. Successful return means only that Windows queued the message: it cannot prove the target consumed it, including on locked, disconnected, secure, or noninteractive sessions. Literal-text payloads therefore always emit a warning. PostMessage cannot trigger global hotkeys registered through `WH_KEYBOARD_LL`, and apps that read raw state through `GetAsyncKeyState` may not observe posted modifiers. Classic Win32/WinForms child-window controls may require the control's own HWND via `-w`/`--target`.
- `send-input` produces real global input (modifiers visible to `GetAsyncKeyState`, low-level hooks fire), but UIPI permits injection only into **equal- or lower-integrity** targets. A normal CLI cannot inject into an elevated app; elevating the CLI does not make `post-message` a bypass, because it follows the same UIPI direction. The command verifies foreground ownership before every semantic action and fails with `foreground_not_target` rather than knowingly typing the next action into the wrong window. A final-instruction foreground race can only be narrowed, not eliminated; keep the target stable and do not run competing focus automation. On a locked, secure, disconnected, or otherwise noninteractive desktop it fails with `no_interactive_desktop`. Before injection it refuses requested virtual keys or any modifier that is already physically held, so ambient Win/Ctrl/Alt/Shift state cannot silently alter the gesture and short-write cleanup owns the key-downs it releases. Cleanup releases only keys still held by the **delivered prefix**; a zero-event write never synthesizes key-up events for keys the CLI did not inject. Physical input after the final state check remains a race.
- **System-reserved combos** (`win+l`, `win+r`, `ctrl+shift+esc`, `ctrl+alt+del`, `alt+tab`, `alt+f4`, `ctrl+esc`, lone `win`/`printscreen`, …) act on the OS/shell when sent globally. `send-input` rejects them by default; `--allow-system-keys` opts in to recoverable cases such as `win+r` or an app global hotkey. An opted-in system shortcut must be the command's **only** key action because it may change foreground; send later input in a separate command after resolving and foregrounding its target again. **Win+L is never bypassable:** the guard covers left/right Win, generic and left/right Shift/Ctrl/Alt extras, aliases/case variants, raw `vk=0x4C`, and the semantic `l` token even when another keyboard layout maps it to a different VK. Immediately before injection it also refuses a payload that could combine with an already-held Win or L key. Physical input can still change after that last state check, so unattended runs should prevent concurrent human/remote keyboard use. `post-message` remains window-queue scoped and cannot trigger the shell's global Win+L hook.
- **Secure Attention Sequence:** `SendInput` cannot synthesize Ctrl+Alt+Delete. Programmatic SAS requires the separate `SendSAS` API and a specially configured service or a signed `uiAccess` app installed in a protected location, plus applicable Windows policy. `winapp ui send-keys` does not call `SendSAS`; `--allow-system-keys` does not grant SAS capability.

Windows API references: [PostMessageW and UIPI](https://learn.microsoft.com/windows/win32/api/winuser/nf-winuser-postmessagew), [SendInput and UIPI](https://learn.microsoft.com/windows/win32/api/winuser/nf-winuser-sendinput), [SendSAS requirements](https://learn.microsoft.com/windows/win32/api/sas/nf-sas-sendsas).
...
- **Literal typed text** (`hello`) differs by transport:
  - `--via send-input` resolves character keys against the **target window thread's keyboard layout** and maps each character to its virtual key (plus Shift), so the target sees a genuine `KeyDown` followed by the OS-composed `WM_CHAR`. Characters not reachable on that layout, or requiring Ctrl/AltGr, fall back to Unicode packets so the exact UTF-16 text still lands; that fallback does not promise the same physical-key/`KeyDown` semantics, especially under an IME. Character main keys in shortcuts are also re-resolved against the target layout at delivery time, preserving case and adding layout-required Shift/Ctrl/AltGr modifiers; an unmappable shortcut character fails with `invalid_arguments` instead of using the CLI process's layout.
  - `--via post-message` posts `WM_CHAR` per character, which does **not** raise a per-character `KeyDown`. Successful return confirms only enqueueing, never consumption, so every literal-text payload emits a warning. WinUI 3 / XAML apps generally do **not** turn a posted `WM_CHAR` into text; classic Win32/WinForms `WM_CHAR`-driven edit controls usually do. **Use `--via send-input` to type literal text into WinUI 3 / WPF apps**; reserve `post-message` typed text for classic Win32 controls or when you only need named keys/combos.
