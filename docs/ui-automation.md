...
>
> **Hardware note.** Touch first verifies that all modern entry points it uses (`CreateSyntheticPointerDevice`, `InjectSyntheticPointerInput`, and `DestroySyntheticPointerDevice`) are exported. If any are unavailable before a frame is accepted, every touch gesture falls back to the legacy `InitializeTouchInjection`/`InjectTouchInput` API when both legacy exports exist. Pen has no legacy equivalent: a missing modern export fails explicitly with **`injection_unsupported`**. If neither touch path is available, or an available API rejects injection, the command reports the API name and stable numeric **`Win32 error N`** semantics rather than `internal_error` or false success.
>
> **Portability and environment.** The preferred touch path and all pen injection require Windows 10 1809+; touch can use its legacy fallback when the running CLI/OS supports it. Callers always use signed absolute desktop physical-pixel coordinates, matching `ui inspect`, UIA, and `GetWindowRect`; foreground and target-window bounds checks stay in that absolute space. For the modern `InjectSyntheticPointerInput` call only, winapp snapshots `SM_XVIRTUALSCREEN`/`SM_YVIRTUALSCREEN` once per gesture and subtracts that virtual-screen origin from pointer locations and touch contact rectangles. The legacy `InjectTouchInput` fallback receives the original absolute coordinates. This supports monitors left of or above the primary display without changing the public contract. The x64 and ARM64 NativeAOT binaries use the same coordinate conversion. Distances are physical pixels, not DIPs, so derive points from `ui inspect` on the target monitor when DPI/scaling differs. Keep the display topology stable during a gesture.
>
...
>
> `--hold-ms` and `--duration-ms` are scheduling targets, not real-time guarantees: system load can delay frames or collapse overdue waits. For automation, branch on stable JSON `error.code` values and the numeric `Win32 error N`. Pointer-injection diagnostics deliberately avoid locale-dependent Win32 message text.
>
> **Failure cleanup is bounded, not atomic.** Cleanup responsibility includes the initial DOWN frame. If a native injection call returns failure, winapp assumes that any contact in that frame might have been accepted and makes one bounded best-effort canceled-UP cleanup for every touch contact (or the pen pointer), using the initial point or last accepted point as appropriate. It also destroys a synthetic device when one was created. The command always exits non-zero. If cancellation or device cleanup also fails, JSON reports that separately in `error.details` (`canceled_up_cleanup_failed` / `device_cleanup_failed`) while preserving the primary failure.
>
> Windows does not guarantee that a failed multi-contact frame was atomically rejected, and a target may already have observed partial input before cancellation. Cleanup is therefore a tested mitigation, not proof of rollback. Verify target UI state before retrying. An `EntryPointNotFoundException`/`DllNotFoundException` occurs before the native frame call; in that specific pre-call case there is no possibly accepted frame, and touch may safely try the legacy path.
...
- `--pressure <0.0–1.0>` — Pen pressure using `.` as the locale-independent decimal separator (default 0.5). `0.0` is preserved exactly as native pressure 0; winapp never silently raises it.
