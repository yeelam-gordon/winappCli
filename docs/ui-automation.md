...
- `post-message` is the default because delivery is scoped to a resolved HWND rather than the global input stream; it does **not** bypass UIPI. A lower-integrity caller cannot post to a higher-integrity target (`ERROR_ACCESS_DENIED` / 5). The CLI checks every `PostMessage` result, returns `input_injection_failed` for UIPI, stale-HWND, or queue failures, and makes a best-effort key-up cleanup if a later message fails after a key-down. Posting is not transactional, so the target may still observe a delivered prefix. It cannot trigger global hotkeys registered through `WH_KEYBOARD_LL`, and apps that read raw state through `GetAsyncKeyState` may not observe posted modifiers. Classic Win32/WinForms child-window controls may require the control's own HWND via `-w`/`--target`.
