...
- `--via <transport>` — `post-message` (default) posts `WM_KEYDOWN`/`WM_KEYUP`/`WM_CHAR` to a resolved target HWND. `send-input` injects OS-wide via `SendInput` and goes to the foreground window. **Both transports are subject to UIPI** and can target only equal- or lower-integrity processes.
...
- `post-message` is the default because delivery is scoped to a resolved HWND rather than the global input stream; it does **not** bypass UIPI. A missing or already-stale HWND fails preflight with `no_target_window`. After a target passes that validation, a later `PostMessage` failure returns `input_injection_failed` — including UIPI denial (`ERROR_ACCESS_DENIED` / 5), the target disappearing in the validation-to-post race, or a queue failure. The CLI checks every post and makes a best-effort key-up cleanup if a later message fails after a key-down. Posting is not transactional, so the target may still observe a delivered prefix. It cannot trigger global hotkeys registered through `WH_KEYBOARD_LL`, and apps that read raw state through `GetAsyncKeyState` may not observe posted modifiers. Classic Win32/WinForms child-window controls may require the control's own HWND via `-w`/`--target`.
- `send-input` produces real global input (modifiers visible to `GetAsyncKeyState`, low-level hooks fire), but UIPI permits injection only into **equal- or lower-integrity** targets. A normal CLI cannot inject into an elevated app; elevating the CLI does not make `post-message` a bypass, because it follows the same UIPI direction. The command verifies foreground ownership twice, including immediately before `SendInput`, and fails with `foreground_not_target` rather than knowingly typing into the wrong window. A final-instruction foreground race can only be narrowed, not eliminated; keep the target stable and do not run competing focus automation. On a locked, secure, disconnected, or otherwise noninteractive desktop it fails with `no_interactive_desktop`. If Windows reports a short write, cleanup releases only keys still held by the **delivered prefix**; a zero-event write never synthesizes key-up events for keys the CLI did not inject.
- **System-reserved combos** (`win+l`, `win+r`, `ctrl+shift+esc`, `ctrl+alt+del`, `alt+tab`, `alt+f4`, `ctrl+esc`, lone `win`/`printscreen`, …) act on the OS/shell when sent globally. `send-input` rejects them by default; `--allow-system-keys` opts in to recoverable cases such as `win+r` or an app global hotkey. **Win+L is never bypassable:** the guard covers left/right Win, generic and left/right Shift/Ctrl/Alt extras, aliases/case variants, raw `vk=0x4C`, and the semantic `l` token even when another keyboard layout maps it to a different VK. Immediately before injection it also refuses a payload that could combine with an already-held Win or L key. Physical input can still change after that last state check, so unattended runs should prevent concurrent human/remote keyboard use. `post-message` remains window-queue scoped and cannot trigger the shell's global Win+L hook.
