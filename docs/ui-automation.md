...
- Human output and JSON `summary.contrast` expose `attempted`, `measured`, and `unmeasured` eligible-text counts. Source-window boundaries are preserved, so a candidate from another top-level window is never sampled against the captured buffer, while child native HWND controls remain eligible for the root capture. Every unmeasured eligible candidate is a failure and makes the command exit non-zero; this includes capture, bounds, cross-window, reliability, and bounded-analysis failures.
