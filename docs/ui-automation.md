...
- `--allow-system-keys` — With `--via send-input`, opt in to system-/shell-reserved global shortcuts that are rejected by default. Plain `win+l` remains blocked; with `post-message` the flag has no effect and emits a warning.
