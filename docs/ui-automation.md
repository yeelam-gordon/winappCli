...
- Contrast analysis uses at most 65,536 deterministic samples per candidate and 4,194,304 samples per audit, plus a fixed luminance histogram instead of allocating or sorting every pixel. Large-text classification estimates rendered size from DPI-normalized element bounds; it does not read the actual font.
...
- Human output and JSON `summary.contrast` expose `attempted`, `measured`, and `unmeasured` eligible-text counts. Per-element native HWND boundaries are preserved; a candidate on an HWND other than the captured root is never sampled against the wrong buffer. Every unmeasured eligible candidate is a failure and makes the command exit non-zero; this includes capture, bounds, cross-HWND, reliability, and bounded-analysis failures.
