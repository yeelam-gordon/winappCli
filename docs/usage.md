...
Contrast is measured only when the `contrast` area is selected; it captures the target window's pixels and samples eligible visible text elements with names and positive bounds. Large-text classification is a DPI-normalized estimate from element bounds, not a font measurement. Eligible elements that cannot be sampled reliably (for example, an element belonging to another window/HWND) produce an unmeasured warning rather than being silently skipped or sampled against the wrong pixels. Non-client chrome (title-bar caption buttons, scrollbar parts) is suppressed, and a defect surfaced by multiple areas is de-duplicated so counts aren't inflated.

Human output and JSON `summary.contrast` report `attempted`, `measured`, and `unmeasured` counts. If eligible text candidates exist but none can be measured—including when capture is unavailable—the command adds an aggregate failure and exits non-zero. Partial coverage reports warnings for each unmeasured candidate. A view with no eligible visible text candidates reports that distinction and does not fail solely for having no contrast candidates.

If no UIA elements at all are discovered, the command reports a failure rather than returning a clean result. `ui audit` is quick heuristic lint, not WCAG or accessibility certification. Supplement it with manual testing and maintained tools such as [Accessibility Insights for Windows](https://accessibilityinsights.io/docs/windows/overview/) or [Axe.Windows](https://github.com/microsoft/axe-windows) for comprehensive rule coverage.
