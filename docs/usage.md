...
Contrast is measured only when the `contrast` area is selected; it captures the target window's pixels and samples eligible visible provider text with positive bounds. Large-text classification is a DPI-normalized estimate from element bounds, not a font measurement. Elements belonging to another window/HWND are never sampled against the wrong pixels: they are marked unmeasured and fail the contrast area. Non-client chrome (title-bar caption buttons, scrollbar parts) is suppressed, and a defect surfaced by multiple areas is de-duplicated so counts aren't inflated.

Human output and JSON `summary.contrast` report `attempted`, `measured`, and `unmeasured` counts. Eligible content includes provider-exposed text and common controls that render their own accessible `Name`/`Value`, with visible child `Text` nodes preferred to avoid duplicate scoring. Contrast uses bounded deterministic sampling and a fixed histogram. Every unmeasured eligible candidate is a failure and exits non-zero; a view with no eligible visible text candidates reports that distinction and does not fail solely for having no contrast candidates.

Audit traversal is cooperatively cancellable and bounded. Depth, element/time limits, and provider child/sibling enumeration failures are explicit audit failures, not clean partial results. Checks occur between provider calls; a UIA COM call already in progress cannot be forcibly interrupted. A supplied selector that no longer resolves returns `element_not_found` and never falls back to the root window. If no UIA elements at all are discovered, the command reports a failure rather than returning a clean result. `ui audit` is quick heuristic lint, not WCAG or accessibility certification. Supplement it with manual testing and maintained tools such as [Accessibility Insights for Windows](https://accessibilityinsights.io/docs/windows/overview/) or [Axe.Windows](https://github.com/microsoft/axe-windows) for comprehensive rule coverage.
