...
- Non-client chrome (title-bar caption buttons, scrollbar parts) is suppressed, and the same defect surfaced by multiple areas is de-duplicated, so counts aren't inflated. Eligible text that cannot be sampled reliably (for example, an element on a different window/HWND) is emitted as an unmeasured warning rather than silently skipped or mis-scored.
