...
- Portability: modern touch and all pen injection require Windows 10 1809+; touch verifies all modern exports and can run every gesture through the legacy `InitializeTouchInjection`/`InjectTouchInput` fallback when both legacy exports exist. Pen has no legacy fallback. Missing APIs return stable `injection_unsupported`, never `internal_error`. Callers, foreground checks, and bounds checks always use signed absolute desktop physical pixels (not DIPs), matching `ui inspect`; modern payloads subtract a once-per-gesture `SM_XVIRTUALSCREEN`/`SM_YVIRTUALSCREEN` origin, while legacy touch keeps the absolute coordinates. This supports monitors left of/above primary on x64 and ARM64 NativeAOT without changing the public contract. Derive points from `ui inspect` on the target monitor and keep display topology stable. Timing options are scheduling targets and can run late under load. Mouse handedness does not remap touch/pen coordinates, but app/Windows gesture settings can affect recognition. Cleanup includes an ambiguous failed initial DOWN: winapp makes one bounded best-effort canceled UP for every possibly accepted pointer at its initial or last accepted point. The OS does not guarantee atomic rejection, so the non-zero exit still means delivery is unconfirmed; verify UI state before retrying. A cleanup failure is preserved separately in JSON `error.details`. Key automation on stable `error.code`, API names, and numeric `Win32 error N`; diagnostics omit locale-dependent Win32 text.
...
```
- `--pressure` 0.0–1.0 using `.` as the locale-independent decimal separator; `0.0` is preserved exactly as native pressure 0 and is never silently raised. `--tilt-x`/`--tilt-y` accept ±90°, and `--eraser` activates the eraser affordance. `--duration-ms` sets total stroke travel time; omitting it uses about 10 ms per path segment. Same injection safety and portability constraints as `touch`: requires a non-zero foregrounded target window and bounds-checks every ink point (out-of-bounds → `invalid_arguments`, nothing injected).
...
|--------|-------------|---------|
| `--app` | Target app (process name, window title, or PID). Lists windows if ambiguous. | (none) |
| `--at` | Explicit start point as signed physical screen coordinates x,y (as reported by 'ui inspect'; values may be negative on secondary monitors). Defaults to the selector's element center. | (none) |
...
| `--json` | Format output as JSON | (none) |
| `--to-point` | End point x,y for a swipe (signed physical screen coordinates). Takes precedence over --direction. | (none) |
| `--window` | Target window by HWND (stable handle from list output). Takes precedence over --app. | (none) |
...
|--------|-------------|---------|
| `--app` | Target app (process name, window title, or PID). Lists windows if ambiguous. | (none) |
| `--at` | Pen contact point as signed physical screen coordinates x,y (as reported by 'ui inspect'; values may be negative on secondary monitors). Defaults to the selector's element center. Ignored when --path is given. | (none) |
| `--duration-ms` | Total glide time in milliseconds distributed across the stroke path segments (default: ~10 ms per segment). | (none) |
| `--eraser` | Activate the pen eraser affordance instead of the normal tip. | (none) |
| `--json` | Format output as JSON | (none) |
| `--path` | Ink stroke path as whitespace-separated signed physical screen x,y pairs, e.g. "10,10 20,30 40,50". | (none) |
| `--pressure` | Pen pressure from 0.0 to 1.0 using '.' as the decimal separator (default: 0.5). | `0.5` |
